...
% vincolo 1
  \draw [domain=-5:58,color=gray!50!black,line width=0.8pt] plot(\x,{(--2200-40*\x)/20});
  \draw [domain=-5:62,color=red!75!black,line width=0.8pt] plot(\x,{(--2400-40*\x)/20});
% vincolo 2
...
% isoprofitto <= 5600
  \draw [line width=0.7pt,dash pattern=on 1pt off 2pt on 3pt off 4pt,color=ffqqtt,domain=-1:48] plot(\x,{(-5600--120*\x)/-40});
% nuova reg. amm.
  \fill[color=red!50!white,opacity=0.3] (25,60)--(20,80)--(10,90)--cycle; %,pattern=north west lines
...
% punto B
  ...
  \fill [color=qqqqff] (20,80) circle (1.5pt);
  \draw[color=qqqqff] (50,80) node {$G\equiv C^\prime \equiv B^\prime$};
% punto E'
  \fill [color=uuuuuu] (60,0) circle (1.5pt);
  \draw[color=uuuuuu] (70,-8) node {$E^\prime$};
% punto I'
  \fill [color=uuuuuu] (0,120) circle (1.5pt);
  \draw[color=uuuuuu] (-5,120) node {$I^\prime$};

  \draw[color=ccqqqq] (120,50) node {$\nabla z$};
  
  \draw[color=gray!50!black] (50, 25) node {$(1)$};
  \draw[color=red] (52, 35) node {$(1^\prime)$};
  \draw[color=uuuuuu] (20,110) node {$(2)$};
  \draw[color=uuuuuu] (70, 40) node {$(3)$};
\end{scriptsize}
  \draw[color=qqqqff] (100,120) node {$B^\prime \equiv x^{\star\prime} = \left|\begin{array}{c}20\\80\end{array}\right|$};
  \draw[color=qqqqff] (110, 80) node {$		z^{\star\prime} = 5600$};
\end{tikzpicture}
\end{document}